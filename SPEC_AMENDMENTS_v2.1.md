...
- **What the add-on contains (implemented July 2026):**
  ...
  5. *Drawing-to-quote* (v1 shipped July 2026) — a customer's 2D PDF drawing (multi-page, incl. scans) is read by vision AI into a structured, human-reviewed extraction; a 3D model is synthesized from it and priced through the normal engine (estimate + G-code). With a STEP file too, a deterministic comparator shows per-feature and envelope mismatches (visually highlighted), lets the user quote the drawing version, the STEP version, or compare both with the price difference. Safety rule: unreadable dimensions become nulls + caveats, never guesses; unconfirmed features are never machined into the model.
  - Roadmap (not in v1): batch RFQ triage across multiple uploads; pockets/slots in drawing synthesis (holes + envelope in v1).
